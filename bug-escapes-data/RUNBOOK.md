...
| `blacklisted_env` | Repeated infra failures (not queue timeout) or environment permanently prevents verification |
| `blacklisted_nd` | Test is non-deterministic — same SHA alternates PASS/FAIL; no reliable signal |
| `backlogged_queue` | Probe run was stuck in queue >2h — cancelled, backlogged, re-examine after 8h |
...
```json
{
  ...
  "backlog": [
    {
      "escape_id": "...",
      "reason": "probe stuck in queue >2h",
      "backlogged_at": "2026-05-21T21:24:00Z",
      "earliest_retry": "2026-05-22T05:24:00Z",
      "original_run_ids": [12345678, 12345679],
      "note": "cancelled — re-examine after earliest_retry"
    }
  ]
}
```
...
- If 0 new candidates → **do NOT exit yet. Escalate lookback** (see Step 1 "Extending lookback when no candidates found"):
  1. Retry with 7 days
  2. If still 0, try bounded time-slice window (-14 to -7 days, then shift farther back each run)
  3. Only exit after max escalation returns 0 candidates:
     ```
     send_message: "No new candidates after extended lookback. Campaign idle. Exiting."
     ```
...
**Extending lookback when no candidates found:**

**Incremental mode (1-day default):** If the 1-day incremental scan returns zero candidates, escalate before giving up:
1. Retry with **7 days** (`DATEADD('day', -7, CURRENT_TIMESTAMP())`).
2. If still empty, try a **bounded time-slice window** (-14 to -7 days) to avoid a slow full scan:
   ```sql
   WHERE p.PIPELINE_START_TS BETWEEN DATEADD('day', -14, CURRENT_TIMESTAMP())
                                   AND DATEADD('day', -7, CURRENT_TIMESTAMP())
   ```
3. Shift the window farther back (e.g., -21/-14, -28/-21) in subsequent cron runs if still empty.
4. Record the window used in `campaign-state.json` under `"search_window"` so future runs know where coverage left off.

**⚠️ 90-day log expiry limit:** GitHub Actions job logs expire after ~90 days. For candidates older than 90 days:
- The Snowflake SQL data (test pass/fail, timestamps, basic failure signatures) still exists and can find candidates
- But full job log inspection is impossible — stack traces, full pytest output, CI environment details are gone
- Skip classification quality degrades significantly: "transient HW", "env issue", etc. cannot be confirmed from logs alone
- **Do NOT dispatch probes for candidates whose regression island (last_fail_ts → first_pass_ts) falls entirely outside the 90-day window** — the original failure context cannot be verified and fix attribution is low-confidence
- The probe runs themselves (BEFORE/AFTER) are always fresh dispatches, so their logs exist regardless of candidate age; the limit applies to candidate *classification* quality only

**Backfill mode (60-day default):** If the initial 60-day backfill returns zero candidates, extend the range before giving up:
1. First retry with **90 days** (`DATEADD('day', -90, CURRENT_TIMESTAMP())`).
2. If still empty (or query is too slow), try a **bounded time-slice window** to cap query cost:
   ```sql
   WHERE p.PIPELINE_START_TS BETWEEN DATEADD('day', -14, CURRENT_TIMESTAMP())
                                   AND DATEADD('day', -7, CURRENT_TIMESTAMP())
   ```
   Shift the window (e.g., -21/-14, -28/-21) to scan farther back without a full 90-day scan.
3. Record the window used in `campaign-state.json` under `"search_window"` so future runs know where coverage left off.
...
- Either run inconclusive due to **ENVIRONMENT** (missing model weights, firmware drift, test not in
  pruned suite, HW SKU unavailable) → **blacklisted_env**: add to `blacklisted[]` in
  `campaign-state.json`, write `{"escape_id": "...", "status": "blacklisted_env", "reason": "..."}` to
  `seen-escapes.json`, do NOT retry (see Blacklisting section)
- Either run inconclusive due to **QUEUE TIMEOUT** (run stuck queued >2h) → **backlogged_queue**: cancel
  the run, add to `backlog[]` in `campaign-state.json` with `earliest_retry = now+8h`, do NOT blacklist
  (see Queue Timeout → Backlog section)
...
**🚨 MANDATORY — Confluence confirmed table (page 2428895268) MUST be updated THE MOMENT an escape is confirmed. Do NOT defer this. Do NOT batch it. Update it before moving on to the next escape.**
URL: https://tenstorrent.atlassian.net/wiki/spaces/MI6/pages/2428895268/Vertical+Bug+Escapes+Confirmed+Table
...
**🚨 MANDATORY — Update Confluence confirmed escapes table (ID 2428895268) IMMEDIATELY:**
https://tenstorrent.atlassian.net/wiki/spaces/MI6/pages/2428895268/Vertical+Bug+Escapes+Confirmed+Table

This is NOT optional and must NOT be deferred. Add a new row to the table the moment verification succeeds (BEFORE=FAIL + AFTER=PASS). Use the same column format as existing entries. The `mcp__jira__confluence_update_page` tool handles versioning internally — call it once with the full updated page content.
...
## Blacklisting (Environmental and ND Failures)

**EVAN'S RULE (2026-05-21):** There is no point wasting time on bug escapes you can't easily verify.
Move on immediately — do not retry — at the FIRST sign of:
- Non-deterministic test behavior (same SHA passes AND fails)
- Repeated infra failures on probe runs (not queue timeouts — see backlog rule below)

Do NOT spend multiple sessions chasing an unverifiable escape. Blacklist or backlog it and move on.

---

### Queue Timeout → Backlog (NOT blacklist)

**EVAN'S RULE (2026-05-21):** Runs stuck in queue are a result of excess runner usage, not a deeper issue. Do NOT blacklist them — backlog them and try again later.

**When a probe run has been in `queued` state for >2 hours:**
1. **Cancel** the queued run immediately (GitHub API: `POST /repos/tenstorrent/tt-metal/actions/runs/{run_id}/cancel`)
2. **Move** the escape from `verification_in_progress` or `bisects_in_progress` to the `backlog` array in `campaign-state.json`:
   ```json
   {
     "escape_id": "...",
     "reason": "probe stuck in queue >2h",
     "backlogged_at": "<ISO timestamp>",
     "earliest_retry": "<backlogged_at + 8 hours>",
     "original_run_ids": [<run_id>, ...],
     "note": "cancelled — re-examine after earliest_retry"
   }
   ```
3. Write `status: "backlogged_queue"` to `seen-escapes.json` for the escape ID
4. **Do NOT examine or re-dispatch** until `earliest_retry` has passed (8h minimum after backlogging)
5. Log to Slack: `🔁 Backlogged {escape_id} — probe stuck in queue >2h, will retry after {earliest_retry}`

**When checking backlog** (session start or watchdog): for each backlog entry, if `now >= earliest_retry`, move it back to `pending` and remove from backlog.

---

**`blacklisted_env`** (infrastructure/environment *permanently* can't support the probe):

⚠️ **QUEUE TIMEOUTS ARE NOT `blacklisted_env`.** A run stuck in the queue for >2h is excess runner usage — use the backlog rule above.

**Decision rule: blacklist after ONE probe confirms env issue.** Do NOT retry. If a single probe attempt shows an environmental failure (both BEFORE and AFTER fail with the same env error, or a structural dependency is missing), blacklist the escape immediately and move on. There is no value in retrying an escape you cannot reliably verify.

`blacklisted_env` triggers on:
- Both BEFORE and AFTER probes fail with the same non-code error (missing model weights, missing HW SKU, CI infra crash unrelated to test code, firmware mismatch, etc.)
- Runner type confirmed permanently offline or unavailable for the entire current campaign window (not just temporarily busy)
- Any escape where the environment structurally prevents verification and a retry would fail for the same structural reason

**`blacklisted_nd`** (test is non-deterministic — no reliable signal):
- Same commit SHA produces **both PASS and FAIL** results across multiple runs
- BEFORE probe produces PASS (should be FAIL) AND prior Snowflake data is inconsistent
- Test is known-flaky (mixed pass/fail in recent nightly history at same SHA)

**Decision rule: blacklist on first confirmed ND observation.** If BEFORE=PASS at the expected-FAIL SHA AND nightly data shows the test also passes at that SHA (confirming it's not a clean regression island), blacklist immediately — do not dispatch further probes.

**Procedure:**
1. Remove the escape from `active_bisects` or `verification_in_progress` in `campaign-state.json`
2. Add an entry to the top-level `blacklisted` list in `campaign-state.json`:
   ```json
   {
     "escape_id": "...",
     "status": "blacklisted_env",   // or blacklisted_nd
     "reason": "one-line human-readable explanation",
     "blacklisted_at": "ISO timestamp"
   }
   ```
3. Write the escape ID to `seen-escapes.json` with the appropriate status value
4. Cancel any associated queued GitHub Actions runs if possible (to free runners)
5. Log to Slack: `⚫ Blacklisted {escape_id} ({reason}) — moving on.`

Do **not** update Confluence for blacklisted escapes (no finding to record).
...
- Any bisect probe in `active_bisects` with probe run stuck `queued` > 2 hours → **cancel run + backlog** (`backlogged_queue`), do NOT blacklist
...
## Horizontal Bug Escape Detection

**Definition:** A horizontal bug escape is a case where a test at layer X fails, and the commit
that fixed it ALSO touches layer X. The bug was introduced and caught at the same architectural
layer. Unlike vertical escapes (fix_layer < test_layer), horizontal escapes indicate that CI
was failing on its own layer before the developer noticed and fixed it.

**Authorized by Evan on 2026-05-25.**

### Constraints (MANDATORY — do not override)

> ⛔ **HARD GATE — READ BEFORE TOUCHING ANY CANDIDATE:**
> If the candidate test ran on T3K, Galaxy, LLMBox, LoudBox, DeskBox, or any multi-card machine,
> **STOP. Discard the candidate immediately.** Do not proceed to Opus classification, do not
> dispatch any probe. The entire horizontal campaign is single-card only. Checking the runner
> label/job name is the FIRST thing to do with every candidate — before any other analysis.

1. **Single-card hardware only.** Horizontal escapes use only N150, N300, P150, P300, BH/Blackhole
   runners. T3K (t3000-wormhole_b0), Galaxy (tg-wormhole_b0, tgg-wormhole_b0), and LLMBox are
   excluded — they are in high demand and must not be consumed for horizontal escape probes.

2. **No ttsim jobs.** Simulator-only failures (jobs whose name contains `ttsim`) are excluded.
   Ttsim timeouts reflect simulator performance regressions, not functional code bugs.

3. **1 verification at a time.** Same rule as vertical: dispatch at most one BEFORE/AFTER probe
   pair before waiting for results.

4. **Confluence: separate section.** Horizontal escape results go in a clearly labelled
   "Horizontal Bug Escapes" section on page 2424012846, separate from vertical escapes.

### Step 1 (Horizontal): Detection SQL

Same islands technique as vertical. Key differences:
- **No `test_layer >= 3` filter** — horizontal escapes are valid at all layers (L1–L4).
- **Single-card host filter** — join to CICD_HOST and restrict to single-card CARD_TYPE values.
- **No ttsim job filter** — exclude jobs whose NAME contains 'ttsim'.
- **Layer comparison for output only** — we compute `test_layer` but do NOT filter by cross-layer
  commits (that's the vertical filter). Instead keep ALL candidates and pass them to Step 2H.
...
WITH runs AS (
  ...
  JOIN TTDATASF.SW_TEST.CICD_JOB j ON t.CICD_JOB_ID = j.CICD_JOB_ID
  JOIN TTDATASF.SW_TEST.CICD_PIPELINE p ON j.CICD_PIPELINE_ID = p.CICD_PIPELINE_ID
  JOIN TTDATASF.SW_TEST.CICD_HOST h ON j.HOST_ID = h.CICD_HOST_ID
  WHERE p.PROJECT = 'tt-metal'
    AND p.GIT_BRANCH_NAME = 'main'
    AND p.PIPELINE_START_TS >= DATEADD('day', -7, CURRENT_TIMESTAMP())  -- 7-day lookback
    AND j.FAILURE_SIGNATURE NOT LIKE 'InfraErrorV1%'
    -- Single-card only: exclude T3K by CARD_TYPE, Galaxy and LLMBox by job name
    -- NOTE: Galaxy WH uses CARD_TYPE='wormhole_b0' and BH-LLMBox uses CARD_TYPE='blackhole'
    -- so CARD_TYPE alone cannot distinguish them from single-card machines. Filter by name.
    AND h.CARD_TYPE NOT IN ('t3000-wormhole_b0', 'tg-wormhole_b0', 'tgg-wormhole_b0')
    AND LOWER(j.NAME) NOT LIKE '%galaxy%'     -- excludes wh_galaxy, bh_galaxy
    AND LOWER(j.NAME) NOT LIKE '%llmbox%'     -- excludes bh_llmbox, BH-LLMBox (multi-card BH)
    AND LOWER(j.NAME) NOT LIKE '%loudbox%'    -- excludes bh_loudbox (8xP150, multi-card BH)
    AND LOWER(j.NAME) NOT LIKE '%deskbox%'    -- excludes BH-DeskBox (2xP150, multi-card BH)
    AND LOWER(j.NAME) NOT LIKE '%multi-card%' -- catch-all for blackhole-multi-card-* job families
    -- Exclude simulator jobs
    AND j.NAME NOT LIKE '%ttsim%'
),
raw_islands AS (
  SELECT *, (rn_asc - rn_by_success) AS island_key
  FROM runs
...
island_summary AS (
  SELECT
    ...
    MAX(rn_asc)                           AS streak_last_rn,
    MAX_BY(job_name, PIPELINE_START_TS)   AS job_name
  FROM raw_islands
  GROUP BY 1, 2, 3
),
fail_streaks AS (
...
transitions AS (
  ...
  JOIN pass_streaks ps
    ON fs.CICD_TEST_CASE_ID = ps.CICD_TEST_CASE_ID
    AND ps.first_pass_rn = fs.last_fail_rn + 1
  WHERE DATEDIFF('hour', fs.last_fail_ts, ps.first_pass_ts) <= 72
),
result AS (
  SELECT
    t.CICD_TEST_CASE_ID,
...
SELECT *
FROM result
-- Note: NO test_layer filter — horizontal escapes valid at all layers (0–4)
ORDER BY last_fail_ts DESC
LIMIT 50;
```

### Step 2H: Same-Layer Commit Filter (replaces vertical Step 2)

For each candidate from Step 1H:
1. Compute escape ID: `{CICD_TEST_CASE_ID}__{last_failing_sha[:8]}`
2. Check `seen-escapes.json`. If present, skip.
3. Get the commit range:
   ```
   GET /repos/tenstorrent/tt-metal/compare/{last_failing_sha}...{first_passing_sha}
   ```
4. Determine `test_layer` from `test_filepath` (same mapping as Step 2 vertical).
5. For each commit in range, determine its lowest touched layer.
6. **Keep the candidate if any commit touches the SAME layer as the test** (`fix_layer == test_layer`).
7. Discard if the ONLY cross-layer commits are at a DIFFERENT layer (vertical or inverse).

**For each same-layer commit, fetch its full diff:**
```
GET /repos/tenstorrent/tt-metal/commits/{sha}
```
Pass `files[].patch` to Opus in Step 3H — same requirement as vertical.

**Commit intent signal**: Note fix-intent keywords in commit message ("fix", "bug", "revert").
Set `low_confidence_prior = true` if ZERO fix-intent commits exist in range.

### Step 3H: Opus Pre-classification (Horizontal variant)

Same noise blocklist (3a) and failure log fetch (3b) as vertical Step 3.

**3cH — Opus prompt (horizontal variant):**

```
You are analyzing a potential HORIZONTAL bug escape in tenstorrent/tt-metal CI.

════════════════════════════════════════════════════════════════
YOUR PRIMARY TASK: DETERMINE WHETHER THIS IS A HORIZONTAL ESCAPE
════════════════════════════════════════════════════════════════

A HORIZONTAL bug escape means a bug at layer N caused a test ALSO at layer N to fail.
The fix commit touches the SAME layer as the failing test.
This is NOT a vertical escape (which would require fix_layer < test_layer).

You must trace a SPECIFIC CAUSAL MECHANISM:
  → Which exact function/macro/API changed in the same-layer commit?
  → What is the call path from that change to the test's execution path?
...
Test: {test_name}
File: {test_filepath} (layer {test_layer})
Streak: {consecutive_fail_count} consecutive failures → {consecutive_pass_count} consecutive passes
Failure window: {last_fail_ts} to {first_pass_ts}
Low-confidence prior: {low_confidence_prior}

Same-layer commits between last failure and first pass (with full diffs):
{commit_list_with_diffs}
...
CHECK 1 — Error classification: Test code or infrastructure?
  → Infrastructure: SKIP_LIKELY_NOISE.

CHECK 2 — Determinism: {consecutive_fail_count} consecutive failures is a strong signal.
  → Inconsistent error signature: SKIP_LIKELY_NOISE.

CHECK 3 — Causal analysis (same-layer, HIGH BAR):
  (a) Specific change: name the exact function/type/constant changed in the fix commit.
  (b) Call path: trace how the test reaches the changed code within the SAME layer.
  (c) Error match: explain why this specific change produces EXACTLY the observed failure.
  → Cannot satisfy all three: SKIP_UNRELATED.

CHECK 4 — Range size: if > 50 commits and no obvious single fix: lower confidence to "low".
...
Respond with JSON only:
{
  ...
  "horizontal_escape_justification": {
    "specific_change": "<exact function/type/constant changed, or null>",
    "call_path": "<how the test reaches the changed code (same layer), or null>",
    "error_match": "<why this specific change produces exactly the observed error, or null>"
  },
  "reasoning": "<2-3 sentences: what the error is, which commit is likely the fix and why>"
}
...
**Post-Opus validation for horizontal:** Inspect `horizontal_escape_justification`.
- If any field is null, generic, or hedged → override to `SKIP_UNRELATED`.
- Mark `status: "skipped_vague_justification"` in `seen-escapes.json`.

**MANDATORY layer check after Opus:** Confirm `most_likely_fix_sha`'s layer EQUALS test layer.
- fix_layer < test_layer → vertical escape, NOT horizontal. Do NOT record as horizontal —
  instead run through the vertical pipeline. Mark `status: "skipped_not_horizontal"`.
- fix_layer > test_layer → inverse, discard. Mark `status: "skipped_not_horizontal"`.
- fix_layer == test_layer → valid horizontal escape. Proceed.

### Steps 4–6 (Horizontal)

**Steps 4–5 are identical to vertical** (bisect, probe dispatch, verification).

Single-card constraint applies: BEFORE/AFTER probes must target the same single-card runner
type as the original failing job. **Never dispatch horizontal probes to T3K, Galaxy, LLMBox,
LoudBox, or DeskBox.** If you reach this step and the original failing job was on a multi-card
machine, you have made an error earlier — discard the candidate now and go back to Step 1H.

**Step 6H — Record Confirmed Horizontal Escape:**

Append to `confirmed-escapes.json` with an additional field:
```json
{
  "escape_id": "...",
  "escape_type": "horizontal",
  "test_layer": "...",
  "fix_layer": "...",   // must equal test_layer for horizontal
  ...
}
```

Validation — same as vertical EXCEPT layer invariant is `fix_layer == test_layer` (not strictly less).

### Step 7H: Confluence (Horizontal — separate section)

All horizontal escape data goes in a **"Horizontal Bug Escapes"** section on page 2424012846,
clearly separated from the vertical "Candidate Table" and "Confirmed Escapes" sections.

Section header (add once if not present):
```
## Horizontal Bug Escapes

Horizontal escapes: the failing test and the fix commit are at the same layer.
Single-card hardware only (N150, N300, P150, P300, BH). No T3K/Galaxy/LLMBox.
```

Candidate table header for horizontal section:
```
| Escape ID | Test File | Job | HW | Last Fail Commit | First Pass Commit | Layer | Runs Dispatched | Opus Verdict | Reasoning | Status |
```

Note: "Layer" column (not "Layers") — just a single layer number (fix_layer == test_layer).
Format: `L{N} (same layer)` e.g. `L3 (same layer)`.

Confirmed horizontal escapes also update the confirmed table (page 2428895268) — add to a
separate "Horizontal" section at the bottom of that table with a matching row format.
Add an `escape_type` column or a note cell to distinguish from vertical entries.
