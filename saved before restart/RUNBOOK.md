...
### Mode A — Backfill
Run once manually. Scans last 90 days. Full analysis pipeline (Steps 1–7 below).
- Consecutive failure threshold: **≥ 10**
- Consecutive pass threshold: **≥ 10**
- Gap threshold: **≤ 72 hours**

### Mode B — Incremental
Scheduled daily at 00:15Z via nanoclaw cron. Scans last 2 days only.
...
fail_streaks AS (
  ...
  FROM island_summary
  WHERE SUCCESS = FALSE
    AND streak_length >= 10        -- 10 for backfill; 3 for incremental
...
transitions AS (
  ...
  WHERE DATEDIFF('hour', fs.last_fail_ts, ps.first_pass_ts) <= 72  -- 72h backfill; 48h incremental
),
result AS (
  SELECT
    t.CICD_TEST_CASE_ID,
...
### Step 3.5: PR-Linked CI Proof Check (NEW)
...
1. For each commit in the SHA range (or just the `most_likely_fix_sha` from Opus), check for an associated PR:
   ```
   GET /repos/tenstorrent/tt-metal/pulls?state=closed&head=tenstorrent:{sha}
   ```
   Or search by SHA: `gh pr list --search {sha} --state merged`

2. For each PR found, check the PR body and comments for GitHub Actions run links (pattern: `https://github.com/tenstorrent/tt-metal/actions/runs/\d+`).

3. For each linked run, check if the test in question was executed and what it returned:
   - Find the job matching the failing test's job name
   - Read the log for explicit PASSED/FAILED

4. **If test PASSES in the linked run AND that run was at the fix commit or after it:**
   - This is direct proof. Record as `confirmation_method: "pr_ci_proof"`, `confidence: "proven"`
   - Use the PR's base SHA as the implicit BEFORE (should be failing there)
   - Skip BEFORE/AFTER probe dispatch entirely
   - Proceed directly to Step 6 (Record)

5. **If test only passes but no BEFORE failure is shown:**
   - Still need a BEFORE probe, but AFTER proof is established
   - Dispatch BEFORE probe only

6. **If no linked run or test not in linked run:** Fall through to Step 4 (bisect).
...
For horizontal candidates, if bisect cannot narrow to a single commit, apply this rule when ALL five conditions hold:
...
3. Bisect was attempted (read existing logs or dispatched probes) but could not isolate a single fix.
...
**After every 5 confirmed escapes (vertical or horizontal):** Update the chart page 2440298505:
...
```
00:15Z  Step 1 (Snowflake, last 2 days, thresholds ≥3/≥3/48h)
00:20Z  Steps 2-3 (layer filter + Opus pre-classification)
00:30Z  Step 3.5 (PR-linked CI proof check)
00:35Z  Step 4 (bisect/probe dispatch for survivors)
03:30Z  Step 4 poll / parse results
04:00Z  Step 5 (verification dispatch for proven vertical candidates)
06:30Z  Step 5 poll / parse verification results
07:00Z  Steps 6-7 (record + Confluence update)
07:05Z  Continuous campaign rule check
```

**Incremental de-duplication:** Check `seen-escapes.json` before processing any candidate. If `test_case_id + last_failing_sha` already exists, skip.
